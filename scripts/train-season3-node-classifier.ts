...
async function main(): Promise<void> {
  ...
  const seed = parseIntEnv('SEASON3_CLASSIFIER_SEED', 42);
  const validationRatio = Math.min(0.4, Math.max(0.1, parseFloatEnv('SEASON3_CLASSIFIER_VAL_RATIO', 0.2)));
  const maxVocabulary = parseIntEnv('SEASON3_CLASSIFIER_MAX_VOCAB', 2000);
  const epochs = parseIntEnv('SEASON3_CLASSIFIER_EPOCHS', 220);
  const learningRate = parseFloatEnv('SEASON3_CLASSIFIER_LR', 0.08);
  const l2 = parseFloatEnv('SEASON3_CLASSIFIER_L2', 0.0005);
  const precisionFloor = Math.min(1, Math.max(0, parseFloatEnv('SEASON3_CLASSIFIER_PRECISION_FLOOR', 0.55)));

  try {
    const dataset = await buildSeason3TrainingDataset(prisma, {
      seasonSlug: 'season-3',
      packSlug: 'sci-fi',
    ...
    const runId = process.env.SEASON3_CLASSIFIER_RUN_ID?.trim() || `season3-node-classifier-${new Date().toISOString()}`;
    ...
    const outputDir = resolve(cli.outputDir ?? `artifacts/season3-node-classifier/${artifact.taxonomyVersion}`);
    ...
    console.log(`Season 3 classifier training complete: taxonomyVersion=${artifact.taxonomyVersion} runId=${artifact.trainingRunId}`);
...
main().catch((error) => {
  console.error('Season 3 classifier training failed');
