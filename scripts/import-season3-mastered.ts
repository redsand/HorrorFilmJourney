...
// Use the current catalog backup or fallback to deterministic
...
function parseCliArgs(): CliOptions {
  ...
  if (inputIndex === -1 || !args[inputIndex + 1]) {
    throw new Error('Missing required flag: --input <path-to-season3-mastered.json>');
  }
  return {
...
function convertV2ToNormalized(payload: MasteredV2): {
  ...
  const taxonomyVersion = payload.taxonomyVersion?.trim() || 'season-3-sci-fi-v2';
  ...
  return {
    season: { slug: 'season-3', name: 'Season 3: Science Fiction' },
    pack: { slug: 'sci-fi', name: 'Sci-Fi' },
    taxonomyVersion,
    nodes,
  };
}
...
async function main(): Promise<void> {
  ...
  if (!isV2Mastered(parsed)) {
    throw new Error('Invalid mastered file shape. Season 3 requires v2 mastered format.');
  }

  const normalized = convertV2ToNormalized(parsed);
  ...
  try {
    ...
    const pack = await prisma.genrePack.upsert({
      ...
      create: {
        ...
        seasonId: season.id,
        primaryGenre: 'sci-fi',
        description: 'From Metropolis to Interstellar — tracing humanity\'s dreams and fears.',
    ...
    for (const node of normalized.nodes) {
      const upsertedNode = await prisma.journeyNode.upsert({
        ...
        create: {
          ...
          learningObjective: `${node.name} concepts and speculative history.`,
          whatToNotice: [],
          eraSubgenreFocus: 'sci-fi',
      ...
      for (const row of node.assignments) {
        ...
        for (const rating of catalogMovie?.ratings ?? []) {
          if (!rating.source || typeof rating.value !== 'number' || !rating.scale) continue;
          await prisma.movieRating.upsert({
            where: { movieId_source: { movieId: persistedMovie.id, source: rating.source } },
            create: {
        ...
        assignments.push({
          ...
          source: 'season3-mastered-import',
          taxonomyVersion: normalized.taxonomyVersion,
        });
      }
    ...
    console.log(
      `Season 3 import complete: input=${inputPath} season=${season.slug} pack=${pack.slug} taxonomyVersion=${normalized.taxonomyVersion} assignments=${importedAssignmentCount} core=${tierCounts.get('CORE') ?? 0} extended=${tierCounts.get('EXTENDED') ?? 0} skippedMissingTmdb=${skippedMissingTmdb} skippedGlobalDuplicateTmdb=${skippedGlobalDuplicateTmdb} activate=${options.activate}`,
    );
  } finally {
...
main().catch((error) => {
  console.error('Season 3 import failed');
